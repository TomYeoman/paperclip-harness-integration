...
**Windows:**
Download the pre-built binary (**v0.25+** required) from the [latest release](https://github.com/datadog-labs/pup/releases/latest) (`pup_*_Windows_x86_64.zip`), extract `pup.exe`, and place it on your PATH.
...
- **Always use `DD_SITE=datadoghq.eu`** — JET uses the EU Datadog site. Set `export DD_SITE=datadoghq.eu` before running pup commands, or pass `--site=datadoghq.eu` on each invocation
...
```bash
# Set site first — required for all pup commands at JET
export DD_SITE=datadoghq.eu

# OAuth2 (preferred) — opens browser, tokens stored in OS keychain
pup auth login
