# JIRA Ticket Template — Feature

> This template is the source of truth. GitHub issue template mirrors it. When the project migrates to JIRA, paste this structure into the JIRA description field.
...
When migrating to JIRA:

1. Paste the full content above into the JIRA description field (markdown is supported)
2. Map GitHub labels → JIRA labels (1:1)
3. Map GitHub assignees → JIRA assignees
4. Link the JIRA ticket back to the closed GitHub issue for traceability
5. BDD doc link and contract test status carry over as-is — no reformatting needed
