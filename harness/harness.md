...
### Part A: Product Discovery (ask one at a time)

These questions define WHAT you're building. Ask them one at a time and wait for each answer before proceeding.

1. **Project name and one-line description**
2. **Who is the primary user?** Give 2-3 examples to anchor the answer (e.g., "college students coordinating hangouts, coworkers planning lunch, families scheduling visits").
3. **What's the core problem in one sentence?** Push back on multi-sentence answers — distill it.
4. **What does the MVP look like?** If the app could only do ONE thing well at launch, what is it?
5. **Walk me through the happy path.** User opens the app — then what? Step by step.
6. **What data does the user see on the main screen?** Be specific — list vs. calendar vs. map, what info per item.
7. **What is explicitly NOT in V1?** Name 3 things people might expect that you're cutting.
8. **Any hard constraints?** (Privacy requirements, platform priority, offline support, etc.)

### Part B: Tech Stack (ask as a batch)

These questions define HOW you're building it. Ask questions 9-14 together as a batch:

9. **Tech stack** (language, framework, platforms, build tool)
10. **How many target platforms?** (e.g., Android + iOS + Web, or just one)
11. **Build commands** — what runs your linter, tests, and full quality check?
12. **What's your DI framework?** (Koin, Dagger/Hilt, manual, none)
13. **What's your test framework?** (JUnit, pytest, Jest, etc.)
14. **Any platform-specific gotchas I should know about?**

### Part C: Infrastructure (ask as a batch)
...
16. **Do you have existing specs/design docs?** If yes, where do they live?

**Default tech recommendations** (use when user says "best practice" or "default"):

- Compose Multiplatform: DI = Koin, Test = kotlin.test + kotlinx-coroutines-test, Build = `./gradlew ktlintFormat` / `./gradlew detekt` / `./gradlew allTests` / `./gradlew check`
- Android-only: DI = Hilt, Test = JUnit5 + Turbine, Build = same as above
- iOS (Swift): DI = manual/Swinject, Test = XCTest, Build = `swift build` / `swift test`
- Web (React/TS): DI = manual/tsyringe, Test = Jest + React Testing Library, Build = `npm run lint` / `npm run test` / `npm run build`
- Python: DI = dependency-injector, Test = pytest, Build = `ruff check --fix` / `mypy .` / `pytest`

### Using Product Answers

After intake, use the product discovery answers (Part A) to:

1. **Generate `docs/PRODUCT-BRIEF.md`** — structured product brief with target user, core problem, MVP scope (in/out), and constraints.
2. **Generate `tasks/MILESTONES.md` with real milestones** — M0 is always project scaffold. M1+ must reflect the actual product from the PO's answers, not generic templates. Each milestone should map to a logical product increment.
3. **Create GitHub issues for M1 tasks** after generation is complete.
...
- agents/roles/ROLE-LEAD.md          (ALL CAPS after ROLE-)
- agents/roles/ROLE-PM.md
- agents/roles/ROLE-BUILDER.md
- agents/roles/ROLE-REVIEWER.md
- agents/roles/ROLE-ARCHITECT.md
- agents/roles/ROLE-TESTER.md
- agents/roles/ROLE-AUDITOR.md
- agents/skills/SKILL-agent-spawn.md (lowercase after SKILL-)
- agents/skills/SKILL-worktree-isolation.md
- agents/skills/SKILL-new-feature-checklist.md
- agents/skills/SKILL-live-learning.md
- agents/skills/SKILL-session-shutdown.md
- agents/skills/SKILL-course-correction.md
- agents/skills/SKILL-pr-review.md
- agents/skills/SKILL-github-pr-workflow.md
- agents/SKILLS-INDEX.md
- agents/SYSTEM-KNOWLEDGE.md
...
- docs/adr/                          (created empty — Architect populates)
- tasks/MILESTONES.md
- tasks/lessons.md                   (lowercase)
...
3. **SESSION START** — Ordered numbered list of what Lead reads at session start:
   1. Read docs/LAUNCH-SCRIPT.md — previous session's handoff
   2. `git fetch origin && git pull origin main`
   3. `git log --oneline -20` — recent changes
   4. Read milestones
   5. If `docs/PRODUCT-BRIEF.md` does not exist → spawn PM agent for product discovery before feature work. Scaffold (M0) may proceed in parallel.
...
2. **Quick Start** — Three steps:
   ...
   2. Say: "Read docs/LAUNCH-SCRIPT.md and begin."
...
3. **Key Files** — Brief table of what to look at:
   ...
   - `docs/LAUNCH-SCRIPT.md` — Current session's startup guide
   - `agents/roles/` — Agent role definitions
   - `agents/skills/` — Lazy-loaded skill files
...
- **Architect never stops** — when Architect finishes M(N) _design_, immediately assign M(N+1)+M(N+2) _design_. Clarify: Architect designs ahead freely, but Builders only start M(N+1) after M(N) completion gates pass. This is not a conflict — Architect produces interfaces, Builders consume them on a separate timeline.
...
- **When spawned**: First launch (no `docs/PRODUCT-BRIEF.md` exists) or milestone boundary needing PO refinement
...
- **Deliverables**: Product brief (`docs/PRODUCT-BRIEF.md`), updated milestones (`tasks/MILESTONES.md`), GitHub issues for M1 tasks
- **NON-NEGOTIABLE block**:
  ...
  - Always create a product brief in docs/PRODUCT-BRIEF.md.
...
- **The enforcement hierarchy** (6 levels, most to least reliable):
  ...
  5. tasks/lessons.md (applied to harness files by Lead)
...
1. **Write lessons** to tasks/lessons.md (format: WHAT_I_DID / WHAT_WAS_WRONG / CORRECTION / PATTERN)
...
4. **Write launch script to `docs/LAUNCH-SCRIPT.md`** — startup guide for next session (overwritten, not appended to build journal)
...
- Open issues
- Remaining milestone tasks ordered by dependency
...
**tasks/lessons.md** — Start with header `# Lessons Log — append only, never edit existing entries` followed by one commented-out example entry showing the exact format:
...
**docs/PRODUCT-BRIEF.md** — Generated from Part A product discovery answers. Contains: one-line description, target user, core problem, MVP scope (in/out), hard constraints, discovery date.

**docs/LAUNCH-SCRIPT.md** — The first-boot launch script generated at install time. Written by the generator as the final step. Overwritten by SKILL-session-shutdown.md at the end of each session. First-boot content:

- Header: "Launch Script — First Session" with current date and M0 as current milestone
- Product Discovery: DONE — see `docs/PRODUCT-BRIEF.md`
- Open PRs: None
...
- Startup Checklist:
  1. Read CLAUDE.md
  2. Read agents/SYSTEM-KNOWLEDGE.md
  3. Read docs/PRODUCT-BRIEF.md — product direction
  4. Read tasks/MILESTONES.md — find M0 tasks
  5. Read agents/SKILLS-INDEX.md (load skills lazily)
  6. Read your role file from agents/roles/
  7. Begin M0

**agents/SYSTEM-KNOWLEDGE.md** — Start with module list skeleton with one filled-in example row to show the expected format. Include a "How to update this file" section at the top explaining that the Architect updates after each milestone.

**agents/SKILLS-INDEX.md** — Pre-populated lookup table with all generated skills. Trigger column must answer "What observable event causes an agent to load this skill?" using the format "When [event]" — not vague temporal references like "before/after". Example: "When about to spawn an agent" not "Before spawning".
...
- **Startup Checklist:**
  ...
  2. Read agents/SYSTEM-KNOWLEDGE.md — module status (Architect populates this)
  3. Read docs/PRODUCT-BRIEF.md — product direction
  4. Read tasks/MILESTONES.md — find M0 tasks
  5. Read agents/SKILLS-INDEX.md — know what skills exist (don't load them yet)
  6. Read your role file from agents/roles/
  7. Begin M0
...
>
> When you relaunch, say: **"Read docs/LAUNCH-SCRIPT.md and begin."**
...
10. **No file references a file that wasn't generated** (note: docs/adr/ is created as an empty directory, not a generated file — references to it are valid)
...
14. **docs/adr/ directory exists** (created during directory setup)
15. **README.md exists** with Quick Start section pointing to `docs/LAUNCH-SCRIPT.md`
